...
function build()
{
    ...
    else if(today.getMonth()+1==1) //  1월 일때
    {
    ...
    for(let i= 2; i<tbcal.rows.length; i++){
        for(let j =0; j<tbcal.rows[i].cells.length; j++){
            tbcal.rows[i].cells[j].addEventListener("click",function(e){
                ...
                day.innerHTML = today.getFullYear()+"년 "+(today.getMonth() + 1)+"월 "+regdate2+"일";
    })
    ...
    document.getElementById("modal_add_btn").onclick = function() {
        ...
        regdate = String(today.getFullYear()) + '-' + String(today.getMonth() + 1)
        day = String(regdate2);
        $.ajax({  
            type: "POST" 
            ,url: "/calenderInsert"
